...
function loadConfig(opts) {
  ...
  const envPort = env.PORT ? Number(env.PORT) : void 0;
...
function encodeId(rootLabel, type, name) {
...
var TRASH_DIR = ".skill-admin-trash";
function timestamp() {
  return (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
}
function rootForPath(roots, real) {
  const sorted = [...roots].sort(
    (a, b) => resolveInRoot(roots, b.path).length - resolveInRoot(roots, a.path).length
  );
  for (const r2 of sorted) {
    const resolvedRoot = resolveInRoot(roots, r2.path);
    const rel = relative2(resolvedRoot, real);
    if (rel === "" || !rel.startsWith("..") && rel !== real) return r2;
  }
...
function createItem(roots, type, rootLabel, name) {
...
function addFile(roots, id, relName, content) {
...
function deleteToTrash(roots, path) {
  const real = resolveInRoot(roots, path);
  if (!existsSync3(real)) throw new FileOpError(`Not found: ${path}`);
  const root = rootForPath(roots, real);
  const resolvedRoot = resolveInRoot(roots, root.path);
  const rel = relative2(resolvedRoot, real);
...
// src/server/api.ts
var ok = (json, status = 200) => ({ status, json });
var err = (status, message) => ({ status, json: { error: message } });
function handleApi(roots, method, url, body) {
  try {
...
var isEntry = process.argv[1] && fileURLToPath(import.meta.url) === normalize(process.argv[1]);
if (isEntry) main();
export {
  createServer,
  main
